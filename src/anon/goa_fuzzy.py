...
class ATGKAnonymization:
    ...
    def _calculate_fitness(self, gorilla: np.ndarray, data: pd.DataFrame, 
        ...
        w_ncp, w_info_loss, w_k_violation, w_pattern = 0.3, 0.2, 0.3, 0.2
        
        return (w_ncp * total_ncp + w_info_loss * information_loss + 
                w_k_violation * k_violation_penalty + w_pattern * pattern_preservation)
    ...
    def _update_position(self, gorilla: np.ndarray, best_gorilla: np.ndarray, 
        ...
        new_position = (gorilla + social_force * exploration_rate +
    ...
    def anonymize(self, data: pd.DataFrame, qis: list[str], k: int) -> pd.DataFrame:
        ...
        print(f"Optimizing with ATGK (iterations: {self.max_iter})...")
        for iteration in range(self.max_iter):
            exploration_rate = 1 - (iteration / self.max_iter)
            
            for i in range(self.n_gorillas):
                fitness = self._calculate_fitness(population[i], processed_data, data, qis, k)
            ...
            for i in range(self.n_gorillas):
                population[i] = self._update_position(population[i], best_gorilla, exploration_rate)
        ...
        for i in range(self.n_clusters):
            ...
            if cluster_size >= k:
                ...
                clusters_anonymized += 1
            else:
                print(f"  Warning: Cluster {i} has only {cluster_size} records (k={k})")
        
        print(f"Anonymization complete: {clusters_anonymized}/{self.n_clusters} clusters anonymized")
        return result
...
def atgk(df: pd.DataFrame, qis: list[str], k: int,
    ...
    print(f"Parameters: gorillas={n_gorillas}, iterations={max_iter}, clusters={n_clusters or max(5, k)}")
    print(f"{'='*60}\n")
    
    anonymizer = ATGKAnonymization(
        n_gorillas=n_gorillas,
        max_iter=max_iter,
        n_clusters=n_clusters or max(5, k),
        m=2.0
    )
